...
PFILE_NAME_INFORMATION Jacker::GetFileName(HANDLE file) {
	ULONG fileNameInfoSize = (2 * MAX_PATH) + sizeof(ULONG);

	PFILE_NAME_INFORMATION fileNameInfo = (PFILE_NAME_INFORMATION)malloc(fileNameInfoSize);
	IO_STATUS_BLOCK ioStatusBlock { 0 };

	NtQueryInformationFile(file, &ioStatusBlock, fileNameInfo, fileNameInfoSize, FileNameInformation);
